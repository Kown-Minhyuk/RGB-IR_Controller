...
void IfxAsclin_Lin_receiveHeader(IfxAsclin_Lin *asclin, uint8 *id)
{
    ...
    IfxAsclin_Lin_waitForReceivedHeader(asclin);    /* waiting until receive header end acknowledgemnet*/
...
void IfxAsclin_Lin_sendResponse(IfxAsclin_Lin *asclin, uint8 *data, uint32 length)
{
    ...
    IfxAsclin_Lin_waitForTransmittedResponse(asclin);    /* wait until transmit response end acknowledgemnet */
}
...
void IfxAsclin_Lin_waitForReceivedResponse(IfxAsclin_Lin *asclin)
{
	...
	sample_time = IfxStm_getLower(stmSfr3);
    ...
    while (asclin->acknowledgmentFlags.rxResponseEnd != 1)
    {
        ...
        current_time = IfxStm_getLower(stmSfr3);
